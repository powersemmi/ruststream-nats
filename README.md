...
- **Core NATS and JetStream, one descriptor each.** `SubscribeOptions` subscribes to a subject and load-balances it across a queue group; `JetStreamConsumer` describes a pull consumer (durable name, filter subject, ack wait, max ack pending, deliver policy, fetch window). Each setting lives on the one model that has it, so asking Core NATS for a durable name does not compile.
