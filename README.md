...
- **Core NATS and JetStream.** Subscribe by subject, or describe a durable JetStream consumer with the `SubscribeOptions` builder (stream, durable name, queue group, filter subject, ack wait, max ack pending, deliver policy, pull expiry).
- **Batches on either transport.** A handler taking `&[T]` names one number at the mount site, the batch size (`b.include(reconcile.batch(nonzero!(6)))`); omitting it on a batch handler is a compile error that says so. On JetStream that number is the pull request's batch size; on Core NATS, which has no wire-level batch, the batches are assembled on the client. Either way the body sees the batch the subscription delivered, never a slice of it, and the mount reads the same.
