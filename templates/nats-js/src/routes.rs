...
//! to a concrete broker only when `main` mounts it. `confirm` carries its `JetStreamSubject`
//! source from the decorator; the registration reuses that source as is. This is the file that
//! names the broker, so it is the one that imports the broker prelude; `orders` names capabilities
//! and imports the core's.
