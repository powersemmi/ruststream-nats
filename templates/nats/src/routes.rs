...
/// commits the registration. The reply travels the default codec unless the chain names one with
/// `.codec(..)`; the runtime pairs the policy with the broker at startup. `on_cancel` has no reply,
/// so its `include` registers on its own.
